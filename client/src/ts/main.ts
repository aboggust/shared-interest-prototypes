...
export function main(el: Element, ignoreUrl: boolean = false, stateParams: Partial<URLParameters> = {}, freezeParams: boolean = false, noSidebar: boolean=false) {
    ...
    const selectors = {
        ...
        method: base.select('.ID_saliency-method-select').classed("empty-sidebar", noSidebar),
    ...
    const eventHelpers = {
        ...
        */
        updateImages: (state: State) => {
            ...
            const imageIDs = api.getImages(state.caseStudy(), state.method(), state.sortBy(), state.predictionFn(), state.scoreFn(),
        ...
        */
        updatePage: (state: State) => {
            // Update histograms
            const allImageIDs = api.getImages(state.caseStudy(), state.method(), state.sortBy(), 'all_images',
                state.scoreFn(), '', [0, 1], [0, 1], [0, 1])
            ...
            // Update image panel
            vizs.saliencyImages.clear()
            const imageIDs = api.getImages(state.caseStudy(), state.method(), state.sortBy(), state.predictionFn(),
                state.scoreFn(), state.labelFilter(), state.iouFilter(), state.groundTruthFilter(),
                state.explanationFilter())
            selectors.body.style('cursor', 'progress')
            imageIDs.then(IDs => {
                // Set the number of images
                state.imageCount(IDs.length)
                ...
                // Set images
                vizs.saliencyImages.update({ caseStudy: state.caseStudy(), method: state.method(), imgIDs: IDs,
                    scoreFn: state.scoreFn()
                })
                selectors.body.style('cursor', 'default')
            })
    ...
    eventHandler.bind(LazySaliencyImages.events.onScreen, ({ el, id, scoreFn, caseStudy, method, caller }) => {
        /* Lazy load the saliency images. */
        const img = new SingleSaliencyImage(el, eventHandler)
        api.getSaliencyImage(caseStudy, method, id, scoreFn).then(salImg => {
            img.update(salImg)
        })
    })
